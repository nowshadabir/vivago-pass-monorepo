...
exports.updateUserProfileName = updateUserProfileName;
exports.deleteVaultItem = deleteVaultItem;
exports.resetPassword = resetPassword;
exports.saveBackupCodes = saveBackupCodes;
exports.verifyAndUseBackupCode = verifyAndUseBackupCode;
exports.getBackupCodesStatus = getBackupCodesStatus;
exports.updateUserPassword = updateUserPassword;
exports.getUserPasskeys = getUserPasskeys;
exports.getPasskeyById = getPasskeyById;
exports.savePasskey = savePasskey;
...
exports.saveSharedItem = saveSharedItem;
exports.getSentSharedItems = getSentSharedItems;
exports.getReceivedSharedItems = getReceivedSharedItems;
    ...
    pool.getConnection()
        ...
        try {
            ...
            `);
                ...
                if (ivCol && ivCol[0] && ivCol[0].Type.includes('varchar')) {
        ...
        `);
            ...
            if (viIvCol && viIvCol[0] && viIvCol[0].Type.includes('varchar')) {
        ...
        `);
            ...
            if (bcIvCol && bcIvCol[0] && bcIvCol[0].Type.includes('varchar')) {
        ...
        `);
            ...
            if (pkIvCol && pkIvCol[0] && pkIvCol[0].Type.includes('varchar')) {
            ...
            `);
                ...
                if (privKeyCol && privKeyCol[0] && privKeyCol[0].Type.includes('varchar')) {
        ...
        `);
            ...
            if (shIvCol && shIvCol[0] && shIvCol[0].Type.includes('varchar')) {
        ...
        `);
            ...
            if (attIvCol && attIvCol[0] && attIvCol[0].Type.includes('varchar')) {
...
async function createUser(id, email, authKeyHash, encryptedMasterKey, masterKeyIv, masterKeyAuthTag) {
    const normalizedEmail = email.trim().toLowerCase();
    ...
    try {
        await pool.query('INSERT INTO users (id, email, name, plan, auth_key_hash, is_verified, encrypted_master_key, master_key_iv, master_key_auth_tag) VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?)', [
            id,
            normalizedEmail,
            name,
            plan,
...
async function updateUserOtp(id, otpCode, expiresAt) {
    if (useFallback || !pool) {
        if (mockUsers[id]) {
...
async function saveVaultItem(userId, item) {
    if (useFallback || !pool) {
    ...
    try {
        await pool.query(`INSERT INTO vault_items (id, user_id, type, name, notes, ciphertext, iv, auth_tag, last_modified) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) 
...
async function updateUserProfileName(id, name) {
    if (useFallback || !pool) {
        if (mockUsers[id]) {
...
async function resetPassword(email, otpCode, newAuthKeyHash, encryptedMasterKey, masterKeyIv, masterKeyAuthTag) {
    const normalizedEmail = email.trim().toLowerCase();
    if (useFallback || !pool) {
        const user = Object.values(mockUsers).find(u => u.email === normalizedEmail);
        if (!user)
    ...
    try {
        ...
        if (user.otp_code === otpCode && Number(user.otp_expires_at) > Date.now()) {
            ...
            try {
                ...
                await connection.query('UPDATE users SET auth_key_hash = ?, otp_code = NULL, otp_expires_at = NULL, encrypted_master_key = ?, master_key_iv = ?, master_key_auth_tag = ? WHERE id = ?', [
                    newAuthKeyHash,
                    toBuffer(encryptedMasterKey),
                    toBuffer(masterKeyIv),
                    toBuffer(masterKeyAuthTag),
    ...
    try {
        ...
        try {
            ...
            for (const c of codes) {
                ...
                await connection.query(`INSERT INTO user_backup_codes (id, user_id, code_hash, encrypted_master_key, iv, auth_tag, is_used)
           VALUES (?, ?, ?, ?, ?, ?, 0)`, [
                    id,
                    userId,
                    c.hash,
...
async function verifyAndUseBackupCode(email, backupCode, markAsUsed = true) {
    const normalizedEmail = email.trim().toLowerCase();
    if (useFallback || !pool) {
        const user = Object.values(mockUsers).find(u => u.email === normalizedEmail);
        if (!user)
        ...
        for (const c of codes) {
            if (c.is_used === 0) {
                const matches = await bcryptjs_1.default.compare(backupCode.trim(), c.code_hash);
                if (matches) {
                    if (markAsUsed) {
                        c.is_used = 1;
                    }
...
async function getBackupCodesStatus(userId) {
    if (useFallback || !pool) {
...
async function updateUserPassword(userId, authKeyHash, encryptedMasterKey, masterKeyIv, masterKeyAuthTag) {
    if (useFallback || !pool) {
    ...
    try {
        await pool.query('UPDATE users SET auth_key_hash = ?, encrypted_master_key = ?, master_key_iv = ?, master_key_auth_tag = ? WHERE id = ?', [
            authKeyHash,
            toBuffer(encryptedMasterKey),
            toBuffer(masterKeyIv),
            toBuffer(masterKeyAuthTag),
...
async function getPasskeyById(id) {
    if (useFallback || !pool) {
...
async function savePasskey(passkey) {
    ...
    try {
        await pool.query(`INSERT INTO user_passkeys (id, user_id, public_key, counter, encrypted_master_key, iv, auth_tag)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            passkey.id,
            passkey.userId,
            passkey.publicKey,
...
async function getUserKeys(userId) {
    ...
    try {
        const [rows] = await pool.query('SELECT public_key, encrypted_private_key, private_key_iv, private_key_auth_tag FROM users WHERE id = ?', [userId]);
        const userRows = rows;
        if (userRows.length === 0)
            return null;
        const row = userRows[0];
...
async function saveUserKeys(userId, publicKey, encryptedPrivateKey, privateKeyIv, privateKeyAuthTag) {
    if (useFallback || !pool) {
        const user = mockUsers[userId];
        if (user) {
    ...
    try {
        await pool.query('UPDATE users SET public_key = ?, encrypted_private_key = ?, private_key_iv = ?, private_key_auth_tag = ? WHERE id = ?', [
            publicKey,
...
async function saveSharedItem(item) {
    ...
    try {
        await pool.query(`INSERT INTO shared_items (id, sender_id, receiver_id, type, name, ciphertext, encrypted_key, iv, auth_tag)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            item.id,
            item.senderId,
            item.receiverId,
...
async function getSentSharedItems(userId) {
    if (useFallback || !pool) {
    ...
    try {
        const [rows] = await pool.query(`SELECT si.*, u.email as receiver_email 
       FROM shared_items si
       JOIN users u ON si.receiver_id = u.id
       WHERE si.sender_id = ?`, [userId]);
        return rows.map(row => ({
            id: row.id,
...
async function getReceivedSharedItems(userId) {
    if (useFallback || !pool) {
        return mockSharedItems
    ...
    try {
        const [rows] = await pool.query(`SELECT si.*, u.email as sender_email 
       FROM shared_items si
       JOIN users u ON si.sender_id = u.id
       WHERE si.receiver_id = ?`, [userId]);
        return rows.map(row => ({
            id: row.id,
            senderId: row.sender_id,
            receiverId: row.receiver_id,
...
async function saveAttachment(attachment) {
    ...
    try {
        await pool.query(`INSERT INTO attachments (id, user_id, vault_item_id, encrypted_metadata, metadata_iv, metadata_auth_tag, file_path, file_size)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            attachment.id,
            attachment.userId,
...
async function deleteAttachment(userId, id) {
    if (useFallback || !pool) {
