...
function validateAuthKey(authKey: any): boolean {
  if (typeof authKey !== 'string') return false;
  const hexRegex = /^[0-9a-fA-F]{64}$/;
  return hexRegex.test(authKey);
}

function validateOtpCode(otpCode: any): boolean {
  if (typeof otpCode !== 'string') return false;
  const otpRegex = /^\d{6}$/;
...
class FilePersistedSessions extends Map<string, string> {
  constructor() {
    super();
    this.load();
  }
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
  ...
  try {
    ...
    const user = await createUser(userId, email, authKeyHash, encryptedMasterKey, masterKeyIv, masterKeyAuthTag);

    // Save backup codes
    if (backupCodes && Array.isArray(backupCodes)) {
      const hashedBackupCodes = await Promise.all(
        backupCodes.map(async (c: any) => {
          const hash = await bcrypt.hash(c.hash.trim(), 10);
          return {
            hash,
      ...
      await saveBackupCodes(user.id, hashedBackupCodes);
    }
...
app.post('/api/auth/verify-otp', otpLimiter, async (req, res) => {
  ...
  try {
    const result = await verifyUserOtp(email, otpCode);
    if (!result.success) {
...
app.post('/api/auth/reset-password', otpLimiter, async (req, res) => {
  ...
  try {
    ...
    const result = await resetPassword(email, otpCode, newAuthKeyHash, encryptedMasterKey, masterKeyIv, masterKeyAuthTag);
...
app.post('/api/auth/login', authLimiter, async (req, res) => {
  ...
  try {
    ...
    // Compare Auth Key
    const isValid = await bcrypt.compare(authKey, user.auth_key_hash);
    if (!isValid) {
      return res.status(401).json({ error: 'Invalid email or password' });
    }
...
app.post('/api/auth/login-backup', authLimiter, async (req, res) => {
  ...
  try {
    const result = await verifyAndUseBackupCode(email, backupCode, false);
    if (!result.success) {
...
app.post('/api/auth/recover-backup', authLimiter, async (req, res) => {
  ...
  try {
    const result = await verifyAndUseBackupCode(email, backupCode);
    if (!result.success) {
      return res.status(401).json({ error: result.error || 'Invalid or used backup code' });
    }

    const newAuthKeyHash = await bcrypt.hash(newAuthKey, 10);
    await updateUserPassword(result.userId!, newAuthKeyHash, encryptedMasterKey, masterKeyIv, masterKeyAuthTag);
...
app.post('/api/user/backup-codes', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await saveBackupCodes(userId, hashedBackupCodes);
...
app.post('/api/vault', requireAuth, async (req, res) => {
  ...
  try {
    const saved = await saveVaultItem(userId, item);
...
app.put('/api/user/profile', requireAuth, async (req, res) => {
  ...
  try {
    await updateUserProfileName(userId, name.trim());
...
app.delete('/api/vault/:id', requireAuth, async (req, res) => {
  ...
  try {
    await deleteVaultItem(userId, itemId);
...
function getWebAuthnConfig(req: express.Request) {
...
app.post('/api/auth/passkey/login-verify', async (req, res) => {
  ...
  try {
    ...
    if (verification.verified) {
      const authInfo = (verification as any).authenticationInfo;
...
app.post('/api/user/keys', requireAuth, async (req, res) => {
  ...
  try {
    await saveUserKeys(userId, publicKey, encryptedPrivateKey, iv, authTag);
    res.json({ success: true });
  } catch (err) {
...
app.get('/api/user/public-key', requireAuth, async (req, res) => {
  ...
  try {
    const result = await getUserPublicKey(email);
    if (!result) {
...
app.post('/api/shares', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const sharedItem = await saveSharedItem({
      id,
      senderId,
      receiverId,
      type,
...
app.get('/api/shares/sent', requireAuth, async (req, res) => {
  const userId = req.headers['x-user-id'] as string;
  try {
    const items = await getSentSharedItems(userId);
    res.json({ items });
  } catch (err) {
...
app.get('/api/shares/received', requireAuth, async (req, res) => {
  const userId = req.headers['x-user-id'] as string;
  try {
    const items = await getReceivedSharedItems(userId);
    res.json({ items });
  } catch (err) {
...
app.post('/api/attachments', requireAuth, upload.single('file'), async (req, res) => {
  ...
  try {
    const attachmentId = path.basename(file.filename, '.enc');
...
app.get('/api/attachments', requireAuth, async (req, res) => {
  const userId = req.headers['x-user-id'] as string;
  try {
    const attachments = await getUserAttachments(userId);
